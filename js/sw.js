...
const isGitHubPages =
  self.location.pathname.startsWith("/website/") ||
  self.location.hostname === "tusharbasak97.github.io";
  ...
  event.waitUntil(
    Promise.all([
      ...
      caches.open(CACHE_NAME).then(async (cache) => {
        ...
        const remainingUrls = STATIC_CACHE_URLS.filter(
          (url) => !criticalPages.includes(url)
        );
      ...
      .then(() => {
        swLog(
          "Service Worker: Installation complete - Enhanced caching enabled"
        );
...
self.addEventListener("fetch", (event) => {
  ...
  if (requestUrl.origin !== self.location.origin) {
    ...
    event.respondWith(
      CacheStrategies.staleWhileRevalidate(event.request, IMAGE_CACHE)
    );
...
const handleNavigationRequest = async (request) => {
  ...
  try {
    ...
    const networkResponse = await CacheStrategies.networkFirst(
      request,
      RUNTIME_CACHE,
      2000
    );
    ...
    swLog(
      "Navigation request failed, checking cache and fallbacks:",
      request.url
    );
...
const isValidRoute = (url) => {
  ...
  const validRoutes = ["/", "/index.html", "/offline.html", "/404.html"];
...
self.addEventListener("sync", (event) => {
  if (event.tag === "background-sync") {
    event.waitUntil(swLog("Service Worker: Background sync triggered"));
  }
});
  ...
  if ("performance" in self) {
    ...
    if (entries.length > 0) {
      ...
      swLog("Service Worker: Performance metrics", {
        ...
        cacheHitRatio:
          self.cacheHitCount / (self.cacheHitCount + self.cacheMissCount) || 0,
      });
    }
  }
};

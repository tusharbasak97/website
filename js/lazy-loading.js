...
(function () {
  ...
  const LAZY_CONFIG = {
    ...
    fallbackDelay: 300,
    retryAttempts: 3,
    placeholderColor: "hsla(0, 0%, 94%, 1.00)",
    fadeInDuration: 300,
  ...
  const utils = {
    ...
    createPlaceholder: (width, height) => {
      ...
      // Add loading text
      ctx.fillStyle = "hsla(0, 0%, 60%, 1.00)";
      ctx.font = "16px Arial";
    ...
    return new Promise((resolve, reject) => {
      ...
      img.onerror = () => {
        if (retries > 0) {
          ...
          setTimeout(() => {
            loadImageWithRetry(src, retries - 1)
              .then(resolve)
              .catch(reject);
          }, 1000);
        } else {
  ...
  const loadImageWithWebPFallback = async (element) => {
    ...
    try {
      ...
      element.src = utils.createPlaceholder(
        element.width || 300,
        element.height || 200
      );
  ...
  const processLazyImage = async (element) => {
    if (
      element.classList.contains("lazy-loading") ||
  ...
  const convertImagesToLazy = () => {
    const images = document.querySelectorAll(
      "img:not([data-src]):not(.no-lazy)"
    );

    images.forEach((img) => {
      if (img.src && !img.src.startsWith("data:")) {
        img.dataset.src = img.src;
        img.src = utils.createPlaceholder(
  ...
  const addLazyLoadingCSS = () => {
    ...
    style.textContent = `
            .lazy-placeholder {
                background: linear-gradient(90deg, hsla(0, 0%, 94%, 1.00) 25%, hsla(0, 0%, 88%, 1.00) 50%, hsla(0, 0%, 94%, 1.00) 75%);
